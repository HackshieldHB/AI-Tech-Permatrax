...
-- AlterEnum: remap legacy PermitPhase values to v2 lifecycle (FIX: data-safe cast)
BEGIN;
CREATE TYPE "PermitPhase_new" AS ENUM (
  'CLUSTER_INTAKE', 'VISIT_REQUEST', 'BA_OPEN', 'SITE_VISIT', 'SURVEY_INPUT', 'ROUTE_SURVEY',
  'BA_SURVEY', 'SIP_REQUEST', 'HLD_SUBMISSION', 'LLD_SUBMISSION', 'PR_BR_ISSUANCE', 'CONTRACT_MANAGEMENT',
  'SKOM_BUDGET', 'MANAGEMENT_APPROVAL', 'FUND_DISBURSEMENT', 'BAK_GENERATION', 'BAKP_COMPILATION',
  'CLAIM_SUBMISSION', 'INVOICE_PACKAGE', 'PERMIT_DONE'
);
ALTER TABLE "PermitCluster" ALTER COLUMN "currentPhase" DROP DEFAULT;
ALTER TABLE "PermitCluster" ALTER COLUMN "currentPhase" TYPE "PermitPhase_new" USING (
  CASE "currentPhase"::text
    WHEN 'APD_DRAFTING' THEN 'HLD_SUBMISSION'::"PermitPhase_new"
    WHEN 'DRM_REVIEW' THEN 'HLD_SUBMISSION'::"PermitPhase_new"
    WHEN 'ABD_SUBMISSION' THEN 'LLD_SUBMISSION'::"PermitPhase_new"
    WHEN 'ABD_REVISION' THEN 'LLD_SUBMISSION'::"PermitPhase_new"
    WHEN 'SOCIALIZATION' THEN 'PR_BR_ISSUANCE'::"PermitPhase_new"
    WHEN 'COMPENSATION_NEGOTIATION' THEN 'CONTRACT_MANAGEMENT'::"PermitPhase_new"
    WHEN 'BAK_APPROVAL' THEN 'BAK_GENERATION'::"PermitPhase_new"
    WHEN 'SIGNATURE_COLLECTION' THEN 'BAK_GENERATION'::"PermitPhase_new"
    WHEN 'SCOM' THEN 'MANAGEMENT_APPROVAL'::"PermitPhase_new"
    WHEN 'BAKP_COMPILATION' THEN 'BAKP_COMPILATION'::"PermitPhase_new"
    WHEN 'BAKP_VALIDATION' THEN 'CLAIM_SUBMISSION'::"PermitPhase_new"
    WHEN 'CONSTRUCTION_READY' THEN 'PERMIT_DONE'::"PermitPhase_new"
    ELSE 'SITE_VISIT'::"PermitPhase_new"
  END
);
...
ALTER TABLE "PermitCluster" ALTER COLUMN "currentPhase" SET DEFAULT 'SITE_VISIT'::"PermitPhase";
COMMIT;

-- CreateTable (remainder from Prisma diff)
